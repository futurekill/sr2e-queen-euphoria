...
PREAMBLE='Use your imagegen skill with the built-in image_gen tool (NOT the CLI fallback). Generate SEAMLESS TILING TEXTURES for a top-down tabletop battle map. CRITICAL REQUIREMENTS: (1) Each image must TILE SEAMLESSLY — the left edge must continue into the right edge and the top into the bottom, with NO visible seam, border, vignette or framing. **DO NOT ACHIEVE THIS BY MIRRORING.** Do not reflect, flip or kaleidoscope one quadrant out to the other three, and do not make the image symmetric about its own centre lines. A mirrored tile is technically seamless and looks like a Rorschach test the moment it repeats — the mirror axes read as hard diagonal lattices across the floor. Every tile generated before this instruction failed exactly this way and had to be thrown out. The left half must NOT be a mirror of the right half, and the top must NOT be a mirror of the bottom: the content has to CONTINUE across the wrap, not reflect at it. It is checked automatically with tools/check-textures.mjs and a mirrored tile is rejected. (2) Perfectly flat TOP-DOWN orthographic view of a floor surface, as if photographed from directly overhead. NO perspective, NO horizon, NO walls, NO objects, NO furniture, NO shadows cast by anything outside the surface. (3) Square 1024x1024. (4) The tile represents roughly 4 x 4 METRES of real floor, so the scale of the material detail must look right at that size — floorboards, carpet pile, tile joints and concrete aggregate all sized accordingly. (5) Gritty near-future Shadowrun realism, slightly grimy and lived-in, dark enough that light-coloured tokens read clearly on top. Save each as webp to the EXACT path given. If a path exists, SKIP it. Report each saved path.'
